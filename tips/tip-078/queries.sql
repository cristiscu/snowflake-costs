...
with cte as (
    select parse_json(system$clustering_information(
        'snowflake_sample_data.tpcds_sf100tcl.store_sales')) json)
select trim(f.path, '[]''') x, f.value::int y
from cte, TABLE(FLATTEN(json:partition_depth_histogram)) f;
